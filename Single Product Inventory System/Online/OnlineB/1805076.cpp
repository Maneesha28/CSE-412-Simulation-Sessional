...
int with_lag_best_s, with_lag_best_S, without_lag_best_s, without_lag_best_S;

double with_lag_best = inf; 
double without_lag_best = inf;
...
void inventory_check(bool lag_order=true) {
    if(lag_order) {
        ...
        if(current_inventory_level < small_s) {
            order_amount = cap_s - current_inventory_level;
            total_order_cost += setup_cost + per_unit_incremental_cost * order_amount;
            event_list[ORDER_ARRIVAL] = simulation_clock;
        }
    }
...
void print_inventory_details() {
    ...
    out << "Policies: " << endl;
    out << "--------------------------------------------------------------------------------------------------" << endl;
    out << " Policy        " << "Avg_TC_With_Lag     " << "Avg_TC_Without_Lag      " << "Better?" << endl;
    out << "--------------------------------------------------------------------------------------------------" << endl;
    out << endl;
...
void run_simulation() {
    ...
    double total_cost_with_lag = total_cost / total_num_of_trials;
        out << "(" << small_s << ", " << cap_s << ")" << "\t\t\t\t" 
    << total_cost_with_lag << "\t\t\t\t";
    ...
    double total_cost_without_lag = total_cost / total_num_of_trials;
    out << total_cost_without_lag << "\t\t\t\t";

    if(total_cost_with_lag >= total_cost_without_lag) {
        out << "Without_Lag" << endl << endl;
    }
    else {
        out << "With_Lag" << endl << endl;
    }

    if(without_lag_best > total_cost_without_lag)
    {
        without_lag_best = total_cost_without_lag;
        without_lag_best_s = small_s;
        without_lag_best_S = cap_s;
    }
    if(with_lag_best > total_cost_with_lag)
    {
        with_lag_best = total_cost_with_lag;
        with_lag_best_s = small_s;
        with_lag_best_S = cap_s;
    }
...
int main() {
    ...
    out << "Optimal policy(s):" << endl << endl;
    out << "[With Lag]: (" << with_lag_best_s << ", " << with_lag_best_S << ")" << endl; 
    out << "[Without Lag]: (" << without_lag_best_s << ", " << without_lag_best_S << ")" << endl; 
