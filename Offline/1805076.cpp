...
enum event_types {EMPTY=0, ARRIVAL=1, DEPARTURE=2};
...
double event_list[3], arrival_time[QUEUE_LIMIT+1]; // indexing from 1
ifstream in;
...
void initialize() {
    ...
    total_num_of_events = 2; //arrival and departure
}
...
void read_input_from_file() {
    ...
    in >> mean_interarrival_time >> mean_service_time >> total_num_of_delays;
    in.close();
}
...
void write_to_result_file() {
    ...
    result_file << "Number of customers: " << total_num_of_delays << endl;
    result_file << endl;
...
{
    initialize();
    while (num_customer_delayed < total_num_of_delays) {
        timing();
